...
import { games as allGames, Game } from '../../data/PcGamesData';
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {visibleGames.map((game) => (
            <div key={game.id} className="bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 transform hover:scale-105">
              ...
              <button className="mt-6 px-4 py-2 bg-indigo-600 text-white rounded-full font-medium shadow-md hover:bg-indigo-700 transition-colors duration-300">
                Show More Details
              </button>
            </div>
          ))}
        ...
        <div className="text-center mt-8">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-lg font-semibold shadow transition-colors duration-300 ${
              startIndex + gamesPerPage >= allGames.length ? "bg-gray-400 text-white cursor-not-allowed" : "bg-indigo-600 text-white hover:bg-indigo-700"
            }`}
          >
            Next
          </button>
